 ...
 * @since       11.1
 */
class jMail_acy extends PHPMailer
{
	/**
	...
	public function Send()
	{
		...
		{
			return acymailing_raiseError(E_NOTICE, 500, acymailing_translation('JLIB_MAIL_FUNCTION_DISABLED'));
		}
		...
		if ($result == false)
		{
			...
			$result = acymailing_raiseError(E_NOTICE, 500, acymailing_translation($this->ErrorInfo));
		}
	...
	public function setSender($from)
	{
		...
		else
		{
			...
			acymailing_raiseError(E_WARNING, 0, acymailing_translation_sprintf('JLIB_MAIL_INVALID_EMAIL_SENDER', $from));
		}
	...
	{
		$subject = acymailing_translation_sprintf('JLIB_MAIL_USER_SUBMITTED', $type);

		$message = sprintf(acymailing_translation('JLIB_MAIL_MSG_ADMIN'), $adminName, $type, $title, $author, $url, $url, 'administrator', $type);
		$message .= acymailing_translation('JLIB_MAIL_MSG') . "\n";

				...
				if(!empty($this->otherSubscriptions) && $this->config->get('unsub_dispothersubs', 0)){
					...
					<div id="unsub_list_div" class="unsubdiv">
						...
						foreach($this->otherSubscriptions as $oneSubscription){
							echo '<div><label for="unsubotherlists'.$i.'"><input type="checkbox" value="1" name="unsubotherlists['.$i.']" id="unsubotherlists'.$i.'" class="unsubotherlistscheckbox"/> '.$oneSubscription->name.'</label>';
							echo '<input type="hidden" value="'.$oneSubscription->listid.'" name="unsubotherlistsid['.$i.']" id="unsubotherlistsid'.$i.'"/></div>';
							$i++;
						}
						?>
					</div>

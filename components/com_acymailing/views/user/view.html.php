	...
	function modify(){
		...
		if(acymailing_level(3)){
			...
			if(!empty($requiredFields)){
				$js = "
				acymailingModule['reqFieldsComp'] = Array('".implode("','", $requiredFields)."');
				acymailingModule['validFieldsComp'] = Array('".implode("','", $validMessages)."');
				";
				acymailing_addScript(true, $js);
			}
			if(!empty($checkFields)){
				$js = "acymailingModule['checkFields'] = Array('".implode("','", $checkFields)."');
				acymailingModule['checkFieldsType'] = Array('".implode("','", $checkFieldsType)."');
				acymailingModule['validCheckFields'] = Array('".implode("','", $validCheckMsg)."');";
				if(!empty($checkFieldsRegexp)) $js .= "acymailingModule['checkFieldsRegexp'] = Array('".implode("','", $checkFieldsRegexp)."');";
				$js .= "
				";
				acymailing_addScript(true, $js);
			}

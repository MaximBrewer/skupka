...
//            $_weight = (isset($t['textdecor']) ? ((strtoupper($t['textdecor'])=='LIGHTER') ? 300 : ((strtoupper($t['textdecor'])=='NORMAL') ? 400 : 700)) : $weight);
    ...
    */
    ...
    function _printFont($name, $tab, $excl = null, $incl=null, $loadDefaultTab = false, $justValue = false){
      ...
      if((!$excl || !in_array('font-weight', $excl)) && (!$incl || in_array('font-weight', $incl)))
        if(isset($t['textdecor']))
          if(!$justValue) echo 'font-weight: '.$t['textdecor'].";\n";
            else echo $t['textdecor'];
      ...
      if((!$excl || !in_array('text-decoration', $excl)) && (!$incl || in_array('text-decoration', $incl))) {
        ...
        if(isset($t['linethrough']))
          if(!$justValue) echo 'text-decoration: '.($t['linethrough'] == '1' ? 'linethrough' : 'none').";\n";
            else echo ($t['linethrough'] == '1' ? 'linethrough' : 'none');
			}

      if((!$excl || !in_array('text-transform', $excl)) && (!$incl || in_array('text-transform', $incl)))
        if(isset($t['uppercase']))
          if(!$justValue) echo 'text-transform: '.($t['uppercase'] == '1' ? 'uppercase' : 'none').";\n";
            else echo ($t['uppercase'] == '1' ? 'uppercase' : 'none');
    ...
    function getTextShadow($s){
      ...
      if (!$ts[4]) return "text-shadow: none;\n";
      while (count($ts) > 4) array_pop($ts);
			foreach ($ts as &$v) {
				if (!is_string($v)) $v = implode('', $v);
			}

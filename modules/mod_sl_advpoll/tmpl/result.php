...
<?php else : ?>
	...
	<ul class="sl_advpoll_graph">
		...
		<?php foreach ($item->answers as $answer) : ?>
			...
			<li>
				...
				<div class="sl_advpoll_answer_votes">
					<?php
						if ($answer->votes  == 0) {
							$text	= 'MOD_SL_ADVPOLL_ZERO_VOTES';
						} else {
							if ($answer->votes > 0) {
								$text	= ($answer->votes ==1) ? 'MOD_SL_ADVPOLL_ONE_VOTES' : 'MOD_SL_ADVPOLL_N_VOTES';
							}
						}
					?>
		...
		<?php if($item->params->get('display_other_answer', 0) == 0) : ?>
			...
			<li>
				<div class="sl_advpoll_answer_title">
					<?php echo JText::sprintf('MOD_SL_ADVPOLL_OTHER_ANSWERS') ?>
				</div>
				...
				<div class="sl_advpoll_answer_votes">
					<?php
						if ($total_other_answer == 0) {
							...
							if ($total_other_answer > 0) {
								$text	= ($total_other_answer ==1) ? 'MOD_SL_ADVPOLL_ONE_VOTES' : 'MOD_SL_ADVPOLL_N_VOTES';
							}
						}
					?>
...
<?php if ($item->params->get('show_votes')) : ?>
	<div class="sl_advppoll_total">
		<?php echo JText::_('MOD_SL_ADVPOLL_TOTAL'); ?>
		<span>
			<?php if ($item->total_votes <= 1) : ?>
				<?php echo JText::sprintf('MOD_SL_ADVPOLL_N_VOTE', $item->total_votes); ?>
			<?php else : ?>
				<?php echo JText::sprintf('MOD_SL_ADVPOLL_N_VOTES', $item->total_votes); ?>
			<?php endif; ?>
		</span>
	</div>
<?php endif; ?>
	...
	<div class="sl_advpoll_msg">
		...
		<div class="sl_advpoll_buttons">
			<button class="sl_advpoll_button" onclick="jQuery.fancybox.close();">
				<?php echo JText::_('MOD_SL_ADVPOLL_CLOSE'); ?>
			</button>
		</div>
	</div>
	<?php endif; ?>

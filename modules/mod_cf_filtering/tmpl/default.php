...
if (! empty($filters_render_array['stylesDeclaration'])) {
    $document->addStyleDeclaration($filters_render_array['stylesDeclaration']);
}
if (! empty($filters_render_array['html'])) {
    $filters_html_array = $filters_render_array['html'];
}
if (isset($filters_render_array['resetUri'])) {
    $resetUri = $filters_render_array['resetUri'];
}
...
<div id="cf_ajax_loader_<?php echo $module->id?>"></div>
	<form method="get" action="<?php echo JRoute::_('index.php?option=com_customfilters&view=products&Itemid='.$Itemid)?>"
		class="cf_form<?php echo $moduleclass_sfx;?>" id="cf_form_<?php echo $module->id?>">
	<?php 
	...
	<div class="cf_flt_wrapper  cf_flt_wrapper_id_<?php echo $module->id?> cf_flt_wrapper_<?php echo $direction ?>"
			id="cf_flt_wrapper_<?php echo $key?>" role="presentation">

...
if(acymailing_level(3)){
	...
	foreach($extraFields as $fieldnamekey => $oneField){
		if(!isset($extraFields[$fieldnamekey]->options)) $extraFields[$fieldnamekey]->options = array();
	...
	if(!empty($checkFields)){
		...
		if(!empty($checkFieldsRegexp)) $js .= "acymailingModule['checkFieldsRegexp".$formName."'] = Array('".implode("','", $checkFieldsRegexp)."');";
	...
	$js = $fieldsClass->prepareConditionalDisplay($arrayForCondDisplay, 'user', 'mod_'.$params->get('displaymode', 'vertical'), $formName);

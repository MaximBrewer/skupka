...
(function () {  // use anonymous wrapper to make sure we do not pollute global namespace whether we use the closure compiler or not
    ...
    */
    ...
    /**
    * Replaces HTML entities with their Unicode equivalents.
    * @param {string} html A string with HTML entities, possibly including `&lt;` and `&gt;`.
    * @return {string} A string with HTML entities replaced with their Unicode equivalents.
    */
    function decodeEntities(html) {
        let txt = /** @type {HTMLTextAreaElement} */ (document.createElement('textarea'));
        txt.innerHTML = html;
        return txt.value;
    }
    ...
    * @param {function(number)} callback Invoked passing the EXIF orientation.
    */
    function getImageOrientationFromURL(url, callback) {
        if (!/\.jpe?g$/i.test(url)) {
            callback(-2);  // wrong image format, no EXIF data present in image formats PNG or GIF
        } else {
    ...
    * @param {function(number):void} callback Invoked passing the EXIF orientation.
    */
    function getImageOrientationFromBlob(blob, callback) {
        ...
        reader.onload = function () {
            ...
            if (view.getUint16(0) != 0xFFD8) {
                callback(-2);  // wrong image format, not a JPEG image
                return;
            }
            ...
            return callback(-1);  // application marker APP1 not found
        };
        ...
        if (isHashChange(href)) {
            ...
            image.addEventListener('load', function (event) {
                ...
                getImageMetadata.call(self, image, function (orientation, metadata) {
                    ...
                    let dpr = /** @type {number} */ (window['devicePixelRatio']) || 1;
                    let h = image.naturalHeight / dpr;
                    let w = image.naturalWidth / dpr;
    ...
    */
    function slideNavigationBar(targetPosition, duration) {
        ...
        navigationStyle.setProperty('transition-duration', (5 * duration) + 'ms');
    }

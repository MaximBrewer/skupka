...
jQuery(document).ready(function($) {
	// jQuery('.addtocart-button',jQuery('#catproduct_form')).click(function(event) {
		// event.preventDefault();
	    // handleToCartOneByOneMy($(this));
		// return false;
	// });
});

//моя функция    старт
function handleToCartOneByOneMy($el) {
    ...
    var product_this_id=$el.data('id');
 	jQuery.when(jQuery("#catproduct_form .product_id").each(function() {
		product_id = jQuery(this).val();
		if(product_id!=0&&Number(product_id)==product_this_id){
			sub_url1 = '';
			noQ = jQuery(this).attr('id').replace('product_id_','');
			if (jQuery("#catproduct_form #virtuemart_product_id"+noQ).attr("type") == "radio") {
				quantity = 0;

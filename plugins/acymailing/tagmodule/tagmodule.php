...
class plgAcymailingTagmodule extends JPlugin{
	...
	function __construct(&$subject, $config){
		...
		if(!isset($this->params)){
			if(!ACYMAILING_J16){
				$params = acymailing_loadResult("SELECT `params` FROM `#__plugins` WHERE `element` = 'tagmodule' AND `folder`= 'acymailing' LIMIT 1");
			}else{
				$params = acymailing_loadResult("SELECT `params` FROM `#__extensions` WHERE `element` = 'tagmodule' AND `type` = 'plugin' AND `folder`= 'acymailing' LIMIT 1");
			}
	...
	function acymailingtagmodule_show(){
		...
		$modules = acymailing_loadObjectList('SELECT id, title, position, module FROM #__modules WHERE ('.implode(') AND (', $filters).') ORDER BY `position`,`ordering`');

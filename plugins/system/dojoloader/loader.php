...
class DojoLoader{
  ...
  function __construct($version = '1.6.1', $scope = 'o'){
	...
	static function getInstance($version = '1.6.1', $scope = 'o'){
		static $instances;
		if (!isset( $instances )) {
			$instances = array();
		}
  ...
  static function addScript($script, $version = null, $scope = 'o'){
    if($version == null)
      $l = DojoLoader::getInstance();
    else
      $l = DojoLoader::getInstance($version, $scope);
  ...
  static function addScriptFile($file, $version = null, $scope = 'o') {
    DojoLoader::addAbsoluteScriptFile(JPATH_SITE.$file, $version, $scope);
  }

  static function addAbsoluteScriptFile($file, $version = null, $scope = 'o'){
  ...
  function checkFolders() {
    ...
    if(isset($get['offlajnclearcache']) || !JFolder::exists($path.DIRECTORY_SEPARATOR.$date)) {

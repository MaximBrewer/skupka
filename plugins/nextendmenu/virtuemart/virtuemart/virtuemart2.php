...
class NextendTreeVirtuemart extends NextendTreebaseJoomla {
    ...
    function initConfig() {
        ...
        $this->_config['blacklist'] = array_filter(explode('||', $this->_data->get('blacklist', '')), "is_numeric");
        ...
        $this->_config['productorder'] = $this->_data->get('order', '0');
    ...
    function getAllItems() {
        ...
        if ($this->_config['displaynum'] || !$this->_config['emptycategory']) {
            ...
            if(VmConfig::get('stockhandle')!='disableit' && VmConfig::get('stockhandle')!='disableit_children') {
        ...
        if(count($this->_config['blacklist']) > 0){
            $query.= " AND a.virtuemart_category_id NOT IN (". implode(',', $this->_config['blacklist']) .") ";
        }
        ...
        if ($this->_config['showproducts']) {
          ...
          SELECT DISTINCT 
            ...
            switch($this->_config['productorder']){
                case 'ordering':
                ...
                case 'name_desc':
                    ...
                    break;
                case 'created_on_asc':
                    $query.="ORDER BY b.created_on ASC";
                    break;

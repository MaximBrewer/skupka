...
$db = JFactory::getDBO();
$db->setQuery("SELECT * FROM `#__virtuemart_userfields` WHERE registration = 1 AND published = 1 AND name NOT IN ('email', 'username', 'name', 'password', 'password2') ORDER BY ordering");
...
foreach ($flds as $key => $v) {
  ...
  if ($v->type == 'select') {
    if ($v->name == 'virtuemart_country_id') {
      ...
      if ($opts) foreach ($opts as $i => $opt) {
        $field[$k]['select']['value'].= '<option value="'.$opt->fieldvalue.'">'.JText::_($opt->fieldtitle).'</option>';
      }
    }
  }
}

...
class emp_rules_userRegistrationRule extends emp_rules_base{
	public function getTrigger(){
		return 'TRIGGER_USER_REGISTRATION';
	}

	public function getTriggerDisplayName(){
		return JText::_('TRIGGER_USER_REGISTRATION');
	}
	...
	public function getDefaultReciepientByType($args){
		...
		if(isset($parameters['preconds']['disabledefaultreciepient']['values']) && $parameters['preconds']['disabledefaultreciepient']['values'] == false){
			if(isset($args['user_name']) && !empty($args['user_name'])){
				$email = $email = $this->getUserEmailFromUserName($args['user_name']);
			}
		}
		return $email;
	}
	...
	public function isSendEmails(){
		return !emp_helper::getGlobalParam('is_disable_registration');
	}
}
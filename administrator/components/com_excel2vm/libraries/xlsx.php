...
if (!class_exists('ZipArchive')) { throw new Exception('Класс ZipArchive не найден. Обратитесь в тех. поддержку хостинга, чтобы они включили данный класс'); }
...
class XLSXWriter
{
	...
	public function writeSheetRow($sheet_name, array $row,$level=0)
	{
		...
		$sheet->file_writer->write('<row collapsed="false" customFormat="false" customHeight="false" hidden="false" ht="12.1" outlineLevel="'.$level.'" r="' . ($sheet->row_count + 1) . '">');
		$column_count=0;
		foreach ($row as $k => $v) {
	...
	public static function xmlspecialchars($val){
        ...
        $val=preg_replace('/([^\pL\pN\pP\pS\pZ])/u', '', $val);
		return preg_replace('/[\x00]/', "", $val);
	}
	//------------------------------------------------------------------

...
define('ACYMAILING_LANGUAGE_FILE', 'com_acymailing');
...
function acymailing_navigationTabs(){
    ...
    $page = str_replace(ACYMAILING_COMPONENT.'_', '', acymailing_getVar('cmd', 'page', ''));
...
function acymailing_loadLanguage(){
	acymailing_loadLanguageFile(ACYMAILING_LANGUAGE_FILE, ACYMAILING_ROOT, null, true);
	acymailing_loadLanguageFile(ACYMAILING_LANGUAGE_FILE.'_custom', ACYMAILING_ROOT, null, true);
}
...
function acymailing_getFilesFolder($folder = 'upload', $multipleFolders = false){
	...
	if(acymailing_isAdmin()){
		...
		if(acymailing_level(1)) $allLists = $listClass->onlyCurrentLanguage($allLists);
		if(acymailing_level(3)) $allLists = $listClass->onlyAllowedLists($allLists);
	}
	$newFolders = array();
...
$config = acymailing_config();
if(!empty($config) && !$config->get('ssl_links', 0)){

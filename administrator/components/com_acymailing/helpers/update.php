...
class acyupdateHelper{
	...
	function addUpdateSite(){
		...
		$query = "SELECT update_site_id FROM #__update_sites WHERE location LIKE '%component-acymailing%' AND type LIKE 'extension'";
		$update_site_id = acymailing_loadResult($query);

		$object = new stdClass();
		$object->name = 'AcyMailing';
		$object->type = 'extension';
		$object->location = 'http://www.acyba.com/component/updateme/updatexml/component-acymailing/version-'.$config->get('version').'/level-'.$config->get('level').'/file-extension.xml';
		...
		$query = "SELECT extension_id FROM #__extensions WHERE `element` LIKE 'com_acymailing' AND type LIKE 'component'";
	...
	function installExtensions(){
		...
		$extensioninfo['plg_system_sendinarticle'] = array('AcyMailing : Send mail while editing an article', 10, 0);

...
class TemplateViewTemplate extends acymailingView{
	...
	function listing(){
		...
		if(acymailing_level(3)){
			$categories = acymailing_loadObjectList('SELECT DISTINCT category FROM #__acymailing_template WHERE category != ""');

			$defaultValue = new stdClass();
			$defaultValue->category = acymailing_translation('ACY_ALL_CATEGORIES');
			array_unshift($categories, $defaultValue);
			$filters->category = acymailing_select($categories, 'category', 'size="1" style="width:150px" onchange="acymailing.submitform();"', 'category', 'category', $pageInfo->category);
		}

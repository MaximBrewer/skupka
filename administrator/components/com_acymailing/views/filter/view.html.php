...
class FilterViewFilter extends acymailingView{
	...
	function form(){
		...
		foreach($data as $datatype){
			...
			if(empty($filter->{$datatype}['type'])) continue;
			foreach($filter->{$datatype}['type'] as $oneFilter){
				if($datatype == 'action'){
				...
				if(!is_array($oneFilter)) continue;
				foreach($oneFilter as $num => $oneType) {
					if(empty($oneType) || !is_int($num)) continue;
					$oneType = htmlspecialchars($oneType, ENT_QUOTES, 'UTF-8');
					...
					foreach($filter->{$datatype}[$num][$oneType] as $key => $value) {
						...
						if (is_array($value)) {
							...
							foreach ($value as $subkey => $subval) {
								$subkey = htmlspecialchars($subkey, ENT_QUOTES, 'UTF-8');
								$subval = htmlspecialchars($subval, ENT_QUOTES, 'UTF-8');

								$js .= 'document.adminForm.elements["' . $datatype . '['.$dataNum.']['.$oneType.']['.$key.']['.$subkey.']"].value = "' . str_replace(array("\n", "\r"), ' ', $subval) . "\";\r\n";

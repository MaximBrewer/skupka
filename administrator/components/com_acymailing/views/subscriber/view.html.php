...
class SubscriberViewSubscriber extends acymailingView{
	...
	function listing(){
		...
		if(acymailing_level(3)){
			...
			$filterFields = acymailing_loadObjectList('SELECT * FROM #__acymailing_fields WHERE '.(acymailing_isAdmin() ? '' : 'front').'listingfilter = 1 AND (type = "'.implode('" OR type = "', $allowedTypes).'")');
			if(!empty($filterFields)){
				$displayedFilters = array();

				foreach($filterFields as $oneField){
					$defaultVal = new stdClass();
					$defaultVal->value = '- '.acymailing_translation($oneField->fieldname).' -';
					$defaultValue = trim($defaultVal->value);
					$arrayValue = array();
					$arrayValue[$defaultValue] = $defaultVal;
					if(!empty($oneField->value)){
						$oneField->value = $customFields->explodeValues($oneField->value);
					}elseif($oneField->type == 'date' || $oneField->type == 'birthday'){
						$values = acymailing_loadObjectList('SELECT DISTINCT `'.$oneField->namekey.'` AS value FROM '.acymailing_table('subscriber').' LIMIT 100');
						if(count($values) == 100){
							$defaultVal->value = '...';
							$values[] = $defaultVal;
						}
						if(!empty($values)){
							$oneField->value = array();
							foreach($values as $onePossibility){
								if(empty($onePossibility->value)) continue;
								$oneField->value[$onePossibility->value] = $onePossibility;
							}
						}
					}

					if(!empty($oneField->value)){
						foreach($oneField->value as $key => $oneVal){
							$arrayValue[$key] = $oneVal;
						}
					}
					$oneField->value = $arrayValue;

					$selectedValue = acymailing_getUserVar($paramBase."fieldfilter_".$oneField->namekey, 'fieldfilter_'.$oneField->namekey, $defaultValue, 'string');
					$selectedValue = trim($selectedValue);

					if($selectedValue != $defaultValue){
						$fieldfilter = true;
						$filters[] = 'a.`'.$oneField->namekey.'` LIKE "%'.acymailing_getEscaped($selectedValue, true).'%"';
					}

					$oneField->type = 'singledropdown';
					$oneField->options = unserialize($oneField->options);
					$oneField->filter_onchange = 'document.adminForm.limitstart.value=0;document.adminForm.submit();';
					$displayedFilters[] = $customFields->display($oneField, $selectedValue, 'fieldfilter_'.$oneField->namekey);
				}
				$this->filterFields = $displayedFilters;
			}
		}
		...
		if(acymailing_level(3)){
			...
			$area = 'backlisting';
			if(!acymailing_isAdmin()) $area = 'frontlisting';
		...
		if(acymailing_isAdmin()){
			...
			if(count($allLists) > 1){
				$filters->lists = acymailing_select($allLists, "filter_lists", 'class="inputbox" size="1" onchange="document.adminForm.limitstart.value=0;document.adminForm.submit();"', 'listid', 'name', intval($selectedList), "filter_lists");
			}else{
				$filters->lists = '<input type="hidden" name="filter_lists" value="'.$selection.'"/>';
			}
		...
		if(acymailing_isAdmin()){
			...
			if(acymailing_isAllowed($config->get('acl_subscriber_import', 'all'))) $acyToolbar->link(acymailing_completeLink('data&task=import&filter_lists='.$selection), acymailing_translation('IMPORT'), 'import');
	...
	function form(){
		...
		if(acymailing_level(3)){
			...
			if(!acymailing_isAdmin()){
				$extraFields = $fieldsClass->getFields('frontform', $subscriber);
			}else{
			...
			if(!empty($js)) acymailing_addScript(true, $js);
		}

		if(!empty($subid) && acymailing_level(2)){
			if(acymailing_isAdmin() && acymailing_isAllowed($config->get('acl_newsletters_send', 'all'))){
				$acyToolbar->popup('send', acymailing_translation('SEND'), acymailing_completeLink("send&task=addqueue&subid=".$subid, true));
				$acyToolbar->divider();
			}
			...
			$clicks = acymailing_loadObjectList($query);
			$this->clicks = $clicks;
		}

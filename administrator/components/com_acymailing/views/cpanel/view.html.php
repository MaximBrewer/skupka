...
class CpanelViewCpanel extends acymailingView{
	...
	function display($tpl = null){
		...
		if(acymailing_level(1)){
			...
			$cronreportval = array();
			$cronreportval[] = acymailing_selectOption('0', acymailing_translation('JOOMEXT_NO'));
			$cronreportval[] = acymailing_selectOption('1', acymailing_translation('ACY_EACH_TIME'));
			$cronreportval[] = acymailing_selectOption('2', acymailing_translation('ACY_ONLY_ACTION'));
			$cronreportval[] = acymailing_selectOption('3', acymailing_translation('ONLY_SOMETHING_WRONG'));
			...
			$jscron .= '}';
			$jscron .= 'document.addEventListener("DOMContentLoaded", function(){ updateCronReport(); });';

			$elements->cron_sendreport = acymailing_select($cronreportval, 'config[cron_sendreport]', 'class="inputbox" size="1" style="width:280px;" onchange="updateCronReport();"', 'value', 'text', (int)$config->get('cron_sendreport', 2), 'cronsendreport');


			$cronsave = array();
			$cronsave[] = acymailing_selectOption('0', acymailing_translation('JOOMEXT_NO'));
			$cronsave[] = acymailing_selectOption('1', acymailing_translation('SIMPLIFIED_REPORT'));
			$cronsave[] = acymailing_selectOption('2', acymailing_translation('DETAILED_REPORT'));
			...
			$jscron .= '}';
			$jscron .= 'document.addEventListener("DOMContentLoaded", function(){ updateCronReportSave(); });';
			acymailing_addScript(true, $jscron);

			$elements->cron_savereport = acymailing_select($cronsave, 'config[cron_savereport]', 'class="inputbox" size="1" style="width:200px;" onchange="updateCronReportSave();"', 'value', 'text', (int)$config->get('cron_savereport', 0), 'cronsavereport');

			$link = acymailing_completeLink('cpanel', true).'&amp;task=cleanreport';
			$elements->deleteReport = acymailing_popup($link, '<button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('REPORT_DELETE').'</button>', '', 400, 100);

			$link = acymailing_completeLink('cpanel', true).'&amp;task=seereport';
			$elements->seeReport = acymailing_popup($link, '<button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('REPORT_SEE').'</button>');

			$link = acymailing_completeLink('email', true).'&amp;task=edit&amp;mailid=report';
			$elements->editReportEmail = acymailing_popup($link, '<button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('REPORT_EDIT').'</button>');

			$elements->cron_url = acymailing_frontendLink('cron', false, false, true);

			$item = $config->get('itemid');
			if(!empty($item)) $elements->cron_url .= '&Itemid='.$item;

			$delayType2 = acymailing_get('type.delay');
			$delayType3 = acymailing_get('type.delay');
			$delayType4 = acymailing_get('type.delay');
			$elements->delete_stats = $delayType2->display('config[delete_stats]', $config->get('delete_stats', 31104000), 4);
			$elements->delete_history = $delayType3->display('config[delete_history]', $config->get('delete_history', 10368000), 4);
			$elements->delete_charts = $delayType4->display('config[delete_charts]', $config->get('delete_charts', 2592000), 4);

			$urlCron = 'https://www.acyba.com/index.php?option=com_updateme&ctrl=launcher&task=edit&component=acymailing&cronurl='.urlencode($elements->cron_url);
			$elements->cron_edit = acymailing_popup($urlCron, '<button class="acymailing_button_grey" onclick="return false">'.acymailing_translation('CREATE_CRON').'</button>');
		}

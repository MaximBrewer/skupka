	...
	<script language="javascript" type="text/javascript">
		...
		function insertTag() {
			...
			<?php if(empty($this->popup)){ ?>
				...
				window.parent.document.getElementById("<?php echo htmlspecialchars($this->controlName.$this->fieldName, ENT_QUOTES, 'UTF-8'); ?>").value = tag;
				window.parent.displayLists();

			<?php }else{ ?>

				var textbox = window.top.document.getElementById("<?php echo htmlspecialchars($this->controlName.$this->fieldName, ENT_QUOTES, 'UTF-8'); ?>");
				textbox.value = tag;
				...
				parent.acymailing.setOnclickPopup("link<?php echo htmlspecialchars($this->controlName.$this->fieldName, ENT_QUOTES, 'UTF-8'); ?>", "<?php echo htmlspecialchars_decode(acymailing_completeLink('chooselist&task='.$this->fieldName.'&control='.$this->controlName)); ?>&values="+tag, 650, 375);

...
class mailClass extends acymailingClass{
	...
	function saveForm(){
		...
		if('joomla' == 'wordpress') {
			$senddateHour   = acymailing_getVar('int', 'senddate_hour', 0);
			if(strlen($senddateHour) < 2) $senddateHour = '0'.$senddateHour;
			$senddateMinute = acymailing_getVar('int', 'senddate_minute', 0);
			if(strlen($senddateMinute) < 2) $senddateMinute = '0'.$senddateMinute;

			$mail->senddate .= $senddateHour.':'.$senddateMinute;
		}

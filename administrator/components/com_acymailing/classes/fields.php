...
class fieldsClass extends acymailingClass{
	...
	function get($fieldid, $default = null){
		...
		$query = 'SELECT a.* FROM '.acymailing_table('fields').' as a WHERE a.`'.$column.'` = '.acymailing_escapeDB($fieldid).' LIMIT 1';
		$field = acymailing_loadObject($query);
	...
	function chart($table, $field){
		...
		if(in_array($field->type, array('checkbox', 'multipledropdown'))){
			...
			foreach($field->value as $valName => $oneValue){
				...
				$myResult = acymailing_loadObject('SELECT COUNT(subid) as total, '.acymailing_escapeDB($valName).' as name FROM '.acymailing_table($table).' WHERE `'.$namekey.'` LIKE '.acymailing_escapeDB('%,'.$valName.',%').' OR `'.$namekey.'` LIKE '.acymailing_escapeDB($valName.',%').' OR `'.$namekey.'` LIKE '.acymailing_escapeDB('%,'.$valName).' OR `'.$namekey.'` = '.acymailing_escapeDB($valName));
				if(!empty($myResult->total)) $results[] = $myResult;
			}
		}else{
			$results = acymailing_loadObjectList('SELECT COUNT(`'.$namekey.'`) as total,'.(($namekey == 'birthday') ? 'YEAR(`'.$namekey.'`)' : ('`'.$namekey.'`')).' as name FROM '.acymailing_table($table).' WHERE `'.$namekey.'` IS NOT NULL'.(($namekey == 'html') ? ' ' : ' AND `'.$namekey.'` != \'\' ').' GROUP BY '.(($namekey == 'birthday') ? 'YEAR(`'.$namekey.'`)' : ('`'.$namekey.'`')).' ORDER BY total DESC LIMIT 20');
		}
		...
		<span id="acy_exportchartlegend" class="acymailing_button" onclick="exportData<?php echo $namekey; ?>();"><?php echo acymailing_translation('ACY_EXPORT'); ?><i class="acyicon-export donotprint" style="cursor:pointer;" alt="<?php echo acymailing_translation('ACY_EXPORT', true) ?>" title="<?php echo acymailing_translation('ACY_EXPORT', true) ?>"></i></span>
	...
	function saveForm(){
		...
		$fieldValues = acymailing_getVar('array', 'fieldvalues', array(), '');
		if(!empty($fieldValues)){
			$field->value = array();
			foreach($fieldValues['value'] as $i => $value){
				$value = trim(str_replace(',', '_', strip_tags($value)));
				$title = strlen($fieldValues['title'][$i]) < 1 ? $value : $fieldValues['title'][$i];
				if(strlen($title) < 1) continue;

				$extra = '';
				if(!empty($fieldValues['disabled'][$i])) $extra .= '::disabled';

				$field->value[] = $value.'::'.$title.$extra;
			}
			$field->value = implode("\n", $field->value);
		}

		$fieldsOptions = acymailing_getVar('array', 'fieldsoptions', array(), '');
		...
		if($field->type == "customtext"){
			$fieldsOptions['customtext'] = acymailing_getVar('string', 'fieldcustomtext', '', '', ACY_ALLOWHTML);
		...
		acymailing_setVar('fieldid', $fieldid);
		return true;
	}
	...
	function delete($elements){
		...
		$fieldsToDelete = acymailing_loadObjectList('SELECT `namekey`,`fieldid` FROM `#__acymailing_fields`  WHERE `core` = 0 AND `fieldid` IN ('.implode(',', $elements).')', 'fieldid');
		if(empty($fieldsToDelete)) return false;

		$allOtherFields = acymailing_loadObjectList('SELECT fieldid, fieldname, options from #__acymailing_fields WHERE fieldid NOT IN ('.implode(',', $elements).')');
		$nameKeyToDel = array();
		...
		foreach($allOtherFields as $oneField){
			...
			do{
				...
				if(!empty($fieldOptions['displim_field'.$i]) && in_array($fieldOptions['displim_field'.$i], $nameKeyToDel)){
					...
					$usedInFields[] = '<a href="'.acymailing_completeLink('fields&task=edit&fieldid='.$oneField->fieldid).'" target="_blank">'.$oneField->fieldname.' ['.acymailing_translation('ACY_ID').' '.$oneField->fieldid.']</a>';
					$used = true;
				}
			}while(!empty($fieldOptions['displim_rel'.$i]));
		}
		if($used){
			acymailing_enqueueMessage(acymailing_translation('ACY_NO_FIELD_DELETED'), 'error');
			foreach($fieldUsed as $k => $oneField){
				acymailing_enqueueMessage(acymailing_translation_sprintf('ACY_FIELD_USED_DISPLAY', '<b>'.$fieldUsed[$k].'</b>', $usedInFields[$k]), 'error');
			}
			return false;
		}
		...
		if(!empty($namekeys)){
			acymailing_query('ALTER TABLE `#__acymailing_subscriber` DROP `'.implode('`, DROP `', $namekeys).'`');
		}


		acymailing_query('UPDATE `#__acymailing_fields` SET fieldcat = 0 WHERE fieldcat IN ('.implode(',', array_keys($fieldsToDelete)).')');

		$affectedRows = acymailing_query('DELETE FROM `#__acymailing_fields` WHERE `fieldid` IN ('.implode(',', array_keys($fieldsToDelete)).')');
		if($affectedRows === false) return false;
	...
	private function _listingFile($field, $value){
		...
		if(empty($path)){
			...
			$path = trim(acymailing_cleanPath(html_entity_decode(acymailing_getFilesFolder())), DS.' ').DS;
	...
	private function _listingGravatar($field, $value){
		if(empty($value) && !empty($this->currentUserEmail)){
			$emailUser = $this->currentUserEmail;
	...
	private function _generateCountryArray(){
		...
		$this->country[''] = acymailing_translation('ACY_PHONE_NOCOUNTRY');
	}
	...
	private function _displayCountry($value, $map, $displayCountry = 1){
		...
		$divCountryCode .= '<span style="position:relative;"><input onkeyup="acymailing_searchACountry('.$id.')" type="text" style="width:100%; margin-bottom:5px;" placeholder="'.acymailing_translation('ACY_SEARCH').'" id="acymailing_searchACountry'.$id.'" class="acymailing_searchACountry" autocomplete="off"></span>';
	...
	function display($field, $value, $map, $inside = false){
		if(!acymailing_isAdmin() && !empty($this->origin) && in_array($this->origin, array('module', 'subform', 'joomla'))){
			if((isset($field->options['editablecreate']) && $field->options['editablecreate'] == 0 && empty($this->currentUserEmail)) || (isset($field->options['editablemodify']) && $field->options['editablemodify'] == 0 && !empty($this->currentUserEmail))){
		...
		if(method_exists($this, $functionType)){
			...
			ob_start();
			$resultTrigger = acymailing_trigger('onAcyDisplayField_'.$field->type, array($field, $value, $map, $inside));
			return ob_get_clean();
		}
	}
	...
	private function _displayGravatar($field, $value, $map, $inside){
		if(empty($value)){
			$url = 'https://www.gravatar.com/avatar/';
			if(!empty($this->currentUserEmail)) $url .= md5(strtolower(trim($this->currentUserEmail)));
			...
			return $img.'<br/>'.$this->_displayFile($field, $value, $map, $inside).'<br/><input type="checkbox" name="'.$map.'" value="" id="delete'.$map.'" /> <label for="delete'.$map.'">'.acymailing_translation('DELETE_PICT').'</label>';
		}
	}
	...
	private function _displayDropdown($field, $value, $map, $type, $inside){
		...
		if(!empty($field->value)){
			foreach($field->value as $oneValue => $myValue){
				...
				$string .= '<option value="'.str_replace(',', '_', htmlspecialchars($oneValue, ENT_COMPAT, 'UTF-8')).'" id="'.$id.'" '.$disabled.' '.$selected.' >'.$this->trans($myValue->value).'</option>'."\n";
			}
		}
		...
		if(!empty($field->options['dbName']) && !empty($field->options['tableName']) && !empty($field->options['valueFromDb']) && !empty($field->options['titleFromDb'])){
			...
			foreach($fieldData as $valueObject){
				...
				$string .= '<option value="'.str_replace(',', '_', htmlspecialchars($valueObject->$valueField, ENT_COMPAT, 'UTF-8')).'" id="'.$id.'" '.$selected.' >'.$this->trans($valueObject->$titleField).'</option>'."\n";
			}
		}
	...
	function _getDataFromDB($field, $valueField, $titleField){
		...
		if($dbName == 'current'){
			$dbName = acymailing_loadResult('SELECT DATABASE()');
		}
		...
		try{
			$res = acymailing_loadObjectList($query);
		}catch(Exception $e){
	...
	private function _displayDate($field, $value, $map, $inside){
		...
		$output = acymailing_calendar($value, $map, $this->prefix.$field->namekey.$this->suffix, $field->options['format'], $styleForCalendar);
	...
	private function _displayBirthday($field, $value, $map, $inside){
		...
		$vals = explode('-', $value);
		$days = array();
		$days[] = acymailing_selectOption('', acymailing_translation('ACY_DAY'));
		for($i = 1; $i < 32; $i++) $days[] = acymailing_selectOption((strlen($i) == 1) ? '0'.$i : $i, $i);
		$years = array();
		$years[] = acymailing_selectOption('', acymailing_translation('ACY_YEAR'));
		for($i = 1901; $i < date('Y') + 10; $i++) $years[] = acymailing_selectOption($i, $i);
		$months = array();
		$months[] = acymailing_selectOption('', acymailing_translation('ACY_MONTH'));
		$months[] = acymailing_selectOption('01', acymailing_translation('ACY_JANUARY'));
		$months[] = acymailing_selectOption('02', acymailing_translation('ACY_FEBRUARY'));
		$months[] = acymailing_selectOption('03', acymailing_translation('ACY_MARCH'));
		$months[] = acymailing_selectOption('04', acymailing_translation('ACY_APRIL'));
		$months[] = acymailing_selectOption('05', acymailing_translation('ACY_MAY'));
		$months[] = acymailing_selectOption('06', acymailing_translation('ACY_JUNE'));
		$months[] = acymailing_selectOption('07', acymailing_translation('ACY_JULY'));
		$months[] = acymailing_selectOption('08', acymailing_translation('ACY_AUGUST'));
		$months[] = acymailing_selectOption('09', acymailing_translation('ACY_SEPTEMBER'));
		$months[] = acymailing_selectOption('10', acymailing_translation('ACY_OCTOBER'));
		$months[] = acymailing_selectOption('11', acymailing_translation('ACY_NOVEMBER'));
		$months[] = acymailing_selectOption('12', acymailing_translation('ACY_DECEMBER'));
		$dayField = acymailing_select($days, $map.'[day]', $class.' style="max-width:75px;" title="'.$this->trans('ACY_DAY').'"', 'value', 'text', @$vals[2], $this->prefix.$field->namekey.$this->suffix.'_day');
		$monthField = acymailing_select($months, $map.'[month]', $class.' style="max-width:75px;" title="'.$this->trans('ACY_MONTH').'"', 'value', 'text', @$vals[1], $this->prefix.$field->namekey.$this->suffix.'_month');
		$yearField = acymailing_select($years, $map.'[year]', $class.' style="max-width:75px;" title="'.$this->trans('ACY_YEAR').'"', 'value', 'text', intval(@$vals[0]), $this->prefix.$field->namekey.$this->suffix.'_year');
	...
	function addJSFunctions(){
		$js = "
			function updateTablesDB(element,dbName){
				...
				xhr.open('GET', '".acymailing_prepareAjaxURL('fields')."&task=updateTablesDB&dbName='+dbName);
				xhr.onload = function(){
			...
			function updateFieldBD(element, dbName, tableName, fieldType, defaultValue){
				var xhr = new XMLHttpRequest();
				xhr.open('GET', '".acymailing_prepareAjaxURL('fields')."&task=updateFieldsDB&dbName='+dbName+'&tableName='+tableName+'&fieldType='+fieldType+'&defaultVal='+defaultValue);
				xhr.onload = function(){
					document.getElementById(element).innerHTML = xhr.responseText;
				};
				xhr.send();
	...
	public function prepareConditionalDisplay($fieldsDisplayed, $prefix, $page, $formid = 'adminForm'){
		...
		$jsEvent .= "
				...
				".$fctName."('".$prefix."');
			";
		$js .= "document.addEventListener(\"DOMContentLoaded\", function(){ ".$jsEvent." });";
		return ($js);
	}

...
class UpdateController extends acymailingController{
	...
	function checkForNewVersion(){
		...
		$config = acymailing_config();
		ob_start();
		$url = ACYMAILING_UPDATEURL.'loadUserInformation&component=acymailing&level='.strtolower($config->get('level', 'starter')).'&version='.$config->get('version', '');
		if(acymailing_level(1)) $url .= '&domain='.urlencode(rtrim(ACYMAILING_LIVE, '/'));

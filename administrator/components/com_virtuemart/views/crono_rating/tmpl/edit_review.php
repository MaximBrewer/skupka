  ...
  <?php } ?>
    ...
    <tr>
        ...
        <td><fieldset>
            <?php
            $db = JFactory::getDbo();
            $users = $db->setQuery('SELECT 0 as id,"Подставной(генерация отзывов)" as name UNION SELECT id, CONCAT(name, "(" ,username,")") as name FROM #__users');
            $users = $db->loadObjectList('id');
            if(!$this->rating->virtuemart_rating_review_id){
                echo JHtmlSelect::genericlist($users, 'created_by', 'class="inputbox"','id','name',$this->rating->created_by);
            }
            else {
                echo isset($users[$this->rating->created_by])?@$users[$this->rating->created_by]->name:'Подставной(генерация отзывов)';
            }
            ?>
            <input type="hidden" name="created_by2" value="<?php echo $this->rating->created_by?>">
        </fieldset></td>
    </tr>
    <tr>
        <td>Товар</td>
        <td><fieldset>
            <?php
            $db->setQuery('SELECT virtuemart_product_id, CONCAT( product_name, "(ID ", virtuemart_product_id, ")" ) as name FROM #__virtuemart_products_'.VMLANG);
            $products = $db->loadObjectList('virtuemart_product_id');
            if(!$this->rating->virtuemart_rating_review_id){
                echo JHtmlSelect::genericlist($products, 'virtuemart_product_id', 'class="inputbox"','virtuemart_product_id','name',$this->rating->virtuemart_product_id);
            }
            else{
                echo $products[$this->rating->virtuemart_product_id]->name;
                echo '<input type="hidden" name="virtuemart_product_id" value="'.$this->rating->virtuemart_product_id.'">';
            }
            ?>
        </fieldset></td>
    </tr>
    <tr>
        ...
        <td>
            <fieldset>
                <input type="text" name="created_on" value="<?php echo $this->rating->created_on;?>">
            </fieldset>
        </td>
    </tr>
</table>

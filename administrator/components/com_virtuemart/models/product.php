 ...
 * @version $Id: product.php 9248 2016-06-30 10:41:15Z Milbo $
 */
...
// 			$joinLang = false;
	...
	public function getNeighborProducts ($product, $onlyPublished = TRUE, $max = 1) {
		...
		$neighbors = array('previous' => '', 'next' => '');
		        ...
		        //выборка продуктов перед и после даного товара $product   старт
				$qm_prev="SELECT p.`virtuemart_product_id`, `l`.`product_name`,`p`.virtuemart_product_id FROM `#__virtuemart_products` as p INNER JOIN `#__virtuemart_products_ru_ru` as l using (`virtuemart_product_id`) LEFT JOIN `#__virtuemart_product_shoppergroups` as ps ON p.`virtuemart_product_id` = `ps`.`virtuemart_product_id`  LEFT JOIN `#__virtuemart_product_categories` as pc ON p.`virtuemart_product_id` = `pc`.`virtuemart_product_id`  WHERE ( `pc`.`virtuemart_category_id` = ".(int)$product->virtuemart_category_id." AND  ( `ps`.`virtuemart_shoppergroup_id`= 1  OR `ps`.`virtuemart_shoppergroup_id` IS NULL  )  AND  p.`published`=1 ) AND p.`virtuemart_product_id`!=".(int)$product->virtuemart_product_id."  AND `p`.virtuemart_product_id <= '".$product->virtuemart_product_id."'  ORDER BY `p`.virtuemart_product_id DESC, `virtuemart_product_id` DESC LIMIT 1";
				$db->setQuery ($qm_prev);
				if ($result = $db->loadAssocList ()) {
					$neighbors['previous'][0]=array("virtuemart_product_id"=>$result[0]['virtuemart_product_id'],'product_name'=>$result[0]['product_name']);
				}

				$qm_next="SELECT p.`virtuemart_product_id`, `l`.`product_name`,`p`.virtuemart_product_id FROM `#__virtuemart_products` as p INNER JOIN `#__virtuemart_products_ru_ru` as l using (`virtuemart_product_id`) LEFT JOIN `#__virtuemart_product_shoppergroups` as ps ON p.`virtuemart_product_id` = `ps`.`virtuemart_product_id`  LEFT JOIN `#__virtuemart_product_categories` as pc ON p.`virtuemart_product_id` = `pc`.`virtuemart_product_id`  WHERE ( `pc`.`virtuemart_category_id` = ".(int)$product->virtuemart_category_id." AND  ( `ps`.`virtuemart_shoppergroup_id`= 1  OR `ps`.`virtuemart_shoppergroup_id` IS NULL  )  AND  p.`published`=1 ) AND p.`virtuemart_product_id`!=".(int)$product->virtuemart_product_id."  AND `p`.virtuemart_product_id >= '".$product->virtuemart_product_id."'  ORDER BY `p`.virtuemart_product_id ASC, `virtuemart_product_id` ASC LIMIT 1";
				$db->setQuery ($qm_next);
				if ($result = $db->loadAssocList ()) {
					$neighbors['next'][0]=array("virtuemart_product_id"=>$result[0]['virtuemart_product_id'],'product_name'=>$result[0]['product_name']);
				}
				return $neighbors;
				//выборка продуктов перед и после даного товара $product   end
...
//vmdebug('my query stuff ',$queryArray);
	...
	*/
	...
	public function createClone ($id) {
		...
		$product->product_name = $product->product_name . '-' . $id; // *!* 2016-07-08, 2016-04-21 Copy/Cloning product
		$product->slug = $product->slug . '-' . $id;
		$product->originId = $id;
//-		$product->published=0; // *!* 2016-07-20 Copy/Cloning product
		$product->published=1; // *!* 2016-07-20 Copy/Cloning product
		...
		//дописуем вставку для плагина customfieldsforall
		$db = JFactory::getDBO();
		$q = 'SELECT * FROM #__virtuemart_product_custom_plg_param_ref WHERE virtuemart_product_id = '.$id;
		$db->setQuery($q);
		$exist_values = $db->loadAssocList();
		foreach ($exist_values as $key => $value) {
			$item = new stdClass();
			$item->virtuemart_product_id = $product->virtuemart_product_id;
			$item->virtuemart_custom_id=$value["virtuemart_custom_id"];
			$item->val=$value["val"];
			$item->intval=$value["intval"];
			$result = $db->insertObject('#__virtuemart_product_custom_plg_param_ref', $item);
		}
		//дописуем вставку для плагина customfieldsforall end
	...
	function getOrderByList ($virtuemart_category_id = FALSE) {
		...
		$uri = JFactory::getURI(); // *!* 2017-04-05 Link when sorting products
		$url = $uri->toString(array('path')); // *!* 2017-04-05 Link when sorting products
		...
		if (count ($fields) > 1) {
			...
			foreach ($fields as $field) {
				if ($field != $orderby) {
					...
					if ( "" != $manufacturerTxt ) $link = $url . "?orderby=" . $field; // *!* 2017-04-05 Link when sorting products
		...
		if ( "" != $manufacturerTxt ) $link = $url; // *!* 2017-04-05 Link when sorting products
		if ( "" != $manufacturerTxt && "" != $orderDirLink ) $link = $url . "?" . substr( $orderDirLink, 1); // *!* 2017-04-05 Link when sorting products

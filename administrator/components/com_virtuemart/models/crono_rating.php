 ...
 * @author RolandD
 */
class VirtueMartModelCrono_Rating extends VmModel {
    public function MenuModelGetItems(){
        $items = array();
        $items[]= array('Отзывы', 'crono_rating', '', 51, 'vmicon vmicon-16-document_move');
        return $items;
    }
...
// 		if(!isset($this->_total)) $this->_total = 0;
    ...
    */
	...
	function getReviews($virtuemart_product_id){
	    ...
	    $whereString = $virtuemart_product_id?' WHERE  `p`.`virtuemart_product_id` = "'.$virtuemart_product_id.'"':'';
    ...
    */
    public function saveRating($data=0) {
		...
		//if ( !empty($data['virtuemart_product_id']) && !empty($userId)){
			...
			if(empty($data)) $data = JRequest::getPost();

 ...
 * @author Max Milbers
 */
class VirtuemartControllerCrono_Rating extends VmController {
...
// 		JRequest::setVar('hidemenu', 1);
	...
	public function listreviews(){
		...
		$view = $this->getView('crono_rating', 'html');
	...
	function storeReview($apply){
        ...
        if(!$data['created_by']){
            $data['created_by'] = $data['created_by2']?$data['created_by2']:-1;
        }
		...
		if($apply){
			$redir = 'index.php?option=com_virtuemart&view=crono_rating&task=edit_review&virtuemart_rating_review_id='.$id;
		} else {
			$virtuemart_product_id = JRequest::getVar('virtuemart_product_id',array(),'', 'array');
			if(is_array($virtuemart_product_id) && count($virtuemart_product_id) > 0){
				$virtuemart_product_id = (int)$virtuemart_product_id[0];
			...
			$redir = 'index.php?option=com_virtuemart&view=crono_rating&task=listreviews&virtuemart_product_id='.$virtuemart_product_id;
		}
        /*crono*/
        if($data['created_by']==-1){
            $q= 'UPDATE  `#__virtuemart_rating_reviews` SET `created_by`= -`virtuemart_rating_review_id` WHERE created_by = -1';
            $db->setQuery($q);
            $db->query();
        }
	...
	function cancelEditReview(){
		...
		$this->setRedirect('index.php?option=com_virtuemart&view=crono_rating&task=listreviews&virtuemart_product_id='.$virtuemart_product_id, $msg);
	}

 ...
 * @since       1.6
 */
class AccordeonckModelItems extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
		...
		if ($assoc)
		{
			...
			$query->join('LEFT', '#__associations AS asso ON asso.id = a.id AND asso.context='.$db->quote('com_accordeonck.item'));
		...
		foreach ($components as $component)
		{
			if ($options = $this->getTypeOptionsByComponent($component->option))
			{
				...
				foreach ($options as $option)
				{
					...
					{
						$this->rlu[MenumanagerckHelper::getLinkKey($option->request)] = $option->get('title');

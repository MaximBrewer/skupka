...
require_once JPATH_COMPONENT.'/helpers/accordeonckitemshelper.php';

/**
 * Menu Item Model for Menus.
 *
 * @package     Joomla.Administrator
 * @subpackage  com_accordeonck
 * @since       1.6
 */
class AccordeonckModelItem extends JModelAdmin
{
	/**
	...
	protected function canEditState($record)
	{
		...
		if (!empty($record->id))
		{
			return $user->authorise('core.edit.state', 'com_accordeonck.item.'.(int) $record->id);
		}
	...
	protected function batchCopy($value, $pks, $contexts)
	{
		...
		$user = JFactory::getUser();
		if (!$user->authorise('core.create', 'com_accordeonck'))
		{
	...
	protected function batchMove($value, $pks, $contexts)
	{
		...
		if (!$user->authorise('core.edit', 'com_accordeonck'))
		{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_accordeonck.item', 'item', array('control' => 'jform', 'load_data' => $loadData), true);
		if (empty($form))
		{
			return false;
		}
	...
	protected function loadFormData()
	{
		...
		return array_merge((array) $this->getItem(), (array) JFactory::getApplication()->getUserState('com_accordeonck.edit.item.data', array()));
	}
	...
	public function getItem($pk = null)
	{
		...
		if ($link = $this->getState('item.link'))
		{
			...
			if (AccordeonckItemsHelper::getLinkKey($table->link) != AccordeonckItemsHelper::getLinkKey($link))
			{
				$table->link = $link;
			}
		}
		...
		switch ($table->type)
		{
			...
			default:
				...
				if (isset($args['option']))
				{
					...
					$table->componentname = $args['option'];
				}
				break;
		}
		...
		if ($assoc)
		{
			if ($pk != null)
			{
				$result->associations = AccordeonckItemsHelper::getAssociations($pk);
			}
			else {
		...
		$result->item_type = $this->getItemType($table);

		return $result;
	}
	
	protected function getItemType($item) {
		if (empty($item->componentname)) return '';
	...
	protected function populateState()
	{
		...
		if (!($parentId = $app->getUserState('com_accordeonck.edit.item.parent_id')))
		{
			$parentId = $app->input->getInt('parent_id');
		}
		$this->setState('item.parent_id', $parentId);

		$menuType = $app->getUserState('com_accordeonck.edit.item.menutype');
		...
		if (!($type = $app->getUserState('com_accordeonck.edit.item.type')))
		{
		...
		if ($link = $app->getUserState('com_accordeonck.edit.item.link'))
		{
			$this->setState('item.link', $link);
		}

		// Load the parameters.
		$params	= JComponentHelper::getParams('com_accordeonck');
	...
	public function save($data)
	{
		...
		$isNew = true;
		$db    = $this->getDbo();
		// JTable::addIncludePath(JPATH_ADMINISTRATOR.'/components/com_accordeonck/tables');
		$table = $this->getTable();
		...
		{
			$table->setLocation($data['parent_id'], 'last-child');
			// set a date alias if needed
			$data['alias'] = $this->createAlias($data['title'], $data['alias'], $data['type']);
		}
		...
		if ($assoc)
		{
			...
			$query->where('context='.$db->quote('com_accordeonck.item'));
			...
			if (!$all_language && count($associations) > 1)
			{
				...
				foreach ($associations as $tag => $id)
				{
					$query->values($id.','.$db->quote('com_accordeonck.item').','.$db->quote($key));
				}
				$db->setQuery($query);
	...
	public function createAlias($alias, $title, $type) {
...
//		$type = $input->post->get('type', '', 'string');
	...
	public function delete(&$pks) {
		$dispatcher = JDispatcher::getInstance(); // from joomla 2.5 compatibility
		$pks = (array) $pks;
		$table = $this->getTable();
		...
		foreach ($pks as $i => $pk)
		{
			...
			if ($table->load($pk))
			{
				...
				else
				{
					...
					{
						return JText::_('JLIB_APPLICATION_ERROR_DELETE_NOT_PERMITTED');
					}
				}
	...
	public function saveparam($id, $param, $value)
	{
		...
		if ($param == 'maximenu_liclass') {
			$value = $this->setLiclass($row->params->get('maximenu_liclass'), $value);
		}
	...
	private function setLiclass($liclass, $btnstate) {

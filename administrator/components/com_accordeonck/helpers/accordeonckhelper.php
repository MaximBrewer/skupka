...
class AccordeonckHelper {
	...
	public static function addSubmenu($vName = '') {
		...
		if (version_compare(JVERSION, '3.0.0') < 0) {
			...
			$doc->addScript(JUri::base(true) . '/components/com_accordeonck/assets/jquery.min.js');
			$doc->addScript(JUri::base(true) . '/components/com_accordeonck/assets/jquery-noconflict.js');
		}
		JSubMenuHelper::addEntry(
				JText::_('CK_MODULES'), 'index.php?option=com_accordeonck&view=modules', $vName == 'modules'
		);
//		JSubMenuHelper::addEntry(
//				JText::_('CK_MENUS'), 'index.php?option=com_accordeonck&view=menus', $vName == 'menus'
//		);
		JSubMenuHelper::addEntry(
				JText::_('CK_ABOUT') . '<span class="accordeonckchecking" data-name="accordeonck" data-type="component" data-folder=""></span>', 'index.php?option=com_accordeonck&view=about', $vName == 'about'
		);
		// JSubMenuHelper::addEntry(
				// JText::_('CK_MIGRATION_TOOL'), 'index.php?option=com_accordeonck&view=migration', $vName == 'migration'
		// );
		$js_checking = 'jQuery(document).ready(function (){
				jQuery(\'.accordeonckchecking\').each(function(i ,el){
				if (jQuery(el).attr(\'data-name\')) {
					jQuery.ajax({
						type: "POST",
						url: \'' . JUri::root(true) . '/administrator/index.php?option=com_accordeonck&task=checkUpdate\',
						data: {
	...
	public static function get_current_version() {
		$db = JFactory::getDbo();
		$db->setQuery('SELECT manifest_cache FROM #__extensions WHERE name = "com_accordeonck"');

 ...
 */
class FavoritesModelJosusers extends JModelList{
	/**
	...
	public function &getData(){
		// Load the data
		if (empty( $this->_data )) {
			$query = 'SELECT * FROM `#__users` WHERE `id` = '.$this->_id;
...
// mcm code 
		...
		$data['block'] = JRequest::getVar('block', '', 'post', 'int');
		$data['sendEmail'] = JRequest::getVar('sendEmail', '', 'post', 'int');
		$data['gid'] = JRequest::getVar('gid', '', 'post', 'int');
		$data['params'] = JRequest::getVar('params', '', 'post', 'string', JREQUEST_ALLOWRAW);
// mcm code 

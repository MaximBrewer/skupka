 ...
 */
class FavoritesModelFavoritesloglist extends JModelList{
	/**
	 * Favoritesloglist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		...
		$query = 'SELECT fav.* FROM `#__virtuemart_favorites_log` fav ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'dt_stamp,log_id,log_type,user_id,cust_id,product_id,product_qty'); // array('id', 'ordering', 'published'); 
	...
	private function _buildQueryWhere() {
		...
		$allowedSearch = explode(',', 'log_type,user_id,cust_id,product_id,product_qty'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
		foreach($allowedSearch as $field){
			if (!$field) return '';
			$where .= " OR (fav.`$field` LIKE '%" . addSlashes($search) . "%') ";
		}
	    return $where;
	}
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__virtuemart_favorites_log` fav ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT log_id AS value `$fieldName` AS text FROM `#__virtuemart_favorites_log` ORDER BY `$fieldName`');
		$options = array();

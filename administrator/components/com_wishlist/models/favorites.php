 ...
 */
class FavoritesModelFavorites extends JModelList{
	/**
	...
	function __construct(){
		...
		$type = 'f';
		if (((int)$array[0]) == 0) {
			$array = JRequest::getVar('uid',  0, '', 'array');
			$type = 'u';
		}
		if (((int)$array[0]) == 0) {
			$array = JRequest::getVar('pid',  0, '', 'array');
			$type = 'p';
		}
		$this->setId((int)$array[0],$type);
	}
	...
	public function &getData(){
		// Load the data
		if (empty( $this->_data )) {
			if ( $this->_type == 'f') $query = 'SELECT * FROM `#__virtuemart_favorites` WHERE `fav_id` = '.$this->_id;
			else if ( $this->_type == 'u') $query = 'SELECT 0 as fav_id, 0 as product_id, -1 as product_qty, '.$this->_id.' as user_id, null as fav_date';
			else if ( $this->_type == 'p') $query = 'SELECT 0 as fav_id, '.$this->_id.' as product_id, -1 as product_qty, 0 as user_id, null as fav_date';
...
// mcm code 
		$data['fav_id'] = JRequest::getVar('id', '', 'post', 'int');

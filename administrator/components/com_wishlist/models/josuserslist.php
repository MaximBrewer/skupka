 ...
 */
class FavoritesModelJosuserslist extends JModelList{
	/**
	 * Josuserslist data array
	 *
	 * @var array
	 */
	private $_data;
	...
	*/
	...
	private function _buildQuery(){
		// use alias usr for easier JOINs writing
		$query = 'SELECT usr.* FROM `#__users` usr ' . $this->_buildQueryWhere() . $this->_buildQueryOrderBy();
		return $query;
	}
	...
	private function _buildQueryOrderBy() {
	    ...
	    $allowedOrders = explode(',', 'id,name,username,email,password,block,sendEmail,gid,registerDate,lastvisitDate,activation,params'); // array('id', 'ordering', 'published'); 
	...
	private function _buildQueryWhere() {
		...
		$allowedSearch = explode(',', 'id,name,username,email'); // array('id', 'ordering', 'published'); 
		$where = ' WHERE (0=1) ';
		foreach($allowedSearch as $field){
			if (!$field) return '';
			$where .= " OR (usr.`$field` LIKE '%" . addSlashes($search) . "%') ";
		}
	    return $where;
	}
	...
	public function getTotal(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT COUNT(*) FROM `#__users` usr ' . $this->_buildQueryWhere() );
	...
	public function &getGenericFieldName($fieldName){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT id AS value `$fieldName` AS text FROM `#__users` ORDER BY `$fieldName`');
		$options = array();
	...
	public function &getIdFieldData(){
		$db =& JFactory::getDBO();
		$db->setQuery( 'SELECT `id` AS value, `id` AS text FROM `#__users` ORDER BY id');
		$options = array();
		foreach( $db->loadObjectList() as $r){
			$options[] = JHTML::_('select.option',  $r->value, $r->text );
		}

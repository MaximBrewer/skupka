 ...
 */
class FavoritesControllerFavoriteslist extends FavoritesController{
	...
	public function edit(){
		JRequest::setVar( 'view', 'favorites' );
	...
	public function save(){
		$model = $this->getModel('favoriteslist'); 
		...
		$link = 'index.php?option=com_wishlist&controller=favoriteslist';
	...
	public function remove(){
		...
		$this->setRedirect( 'index.php?option=com_wishlist&controller=favoriteslist', $msg );
	}
	...
	public function publish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__virtuemart_favorites SET published = 1 WHERE `fav_id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
		$this->setMessage( JText::sprintf('Fields published', $n ) );
		$this->setRedirect( 'index.php?option=com_wishlist&controller=favoriteslist' );
	}
	...
	public function unpublish(){
		...
		$cids = implode( ',', $cid );
		$db	=& JFactory::getDBO();
		$query = 'UPDATE #__virtuemart_favorites SET published = 0 WHERE `fav_id` IN ( '. $cids.'  )';
		$db->setQuery( $query );
		if (!$db->query()) {
			return JError::raiseWarning( 500, $db->getError() );
		}
	...
	function cancel() {
		$model = $this->getModel( 'favorites' );
		$model->checkin();
		$this->setRedirect( 'index.php?option=com_wishlist&view=favoriteslist' );
	}
	*/

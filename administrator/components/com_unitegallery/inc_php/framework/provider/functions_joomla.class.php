...
defined('_JEXEC') or die('Restricted access');
	...
	class UniteFunctionJoomlaUG{
		...
		public static function putHtmlFieldsetBox($form,$name,$boxTitle="Settings"){
			...
			if(empty($form))
				UniteFunctionsUG::throwError("Form not found!!!");
			...
			?>
				...
				<fieldset class="adminform unite-adminform">
					...
					<?php
						...
						if(empty($fieldset))
							UniteFunctionsUG::throwError("fieldset with name: $name not found.");
		...
		public static function getImageOutputUrl($filename,$width=0,$height=0,$exact=false,$encode=true){
			...
			if(($exact == "true" || $exact == true) && (empty($width) || empty($height) ))
				UniteFunctionsUG::throwError("Exact must have both - width and height");
		...
		public static function getImageFilepathFromUrl($urlImage){
			...
			$filepathImage = GlobalsUG::$path_base.$pathImage;
		...
		public static function isJqueryIncluded(){
			...
			foreach($arrScripts as $filepath=>$arrDetails){
				...
				$pattern = '#jquery([0-9\.-]|core|min|pack)*?.js#';
		...
		public static function addjQueryInclude($app, $urljQuery = null){
			...
			$isJoomla3 = self::isJoomla3();
			if($isJoomla3 == true){
				JHtml::_('jquery.framework');
				return(false);
			}
			
			if(self::isJqueryIncluded() == true)
				return(false);
			...
			if(!empty($urljQuery))
				$document->addScript($urljQuery."?app={$app}");
			else
				$document->addScript("{$jsPrefix}://ajax.googleapis.com/ajax/libs/jquery/1.10.2/jquery.min.js?app={$app}");

			...
			<table class="table table-striped" id="rulesList">
				...
				<tbody>
					...
					foreach ($this->items as $i => $item) :
						$canCheckin = $loggeduser->authorise('core.manage', 'com_checkin') || $item->locked_by == $userId || $item->locked_by == 0;
						...
						<tr>
							...
							<td>
								<div class="name break-word">
									<?php if ($item->locked_by) : ?>
										<?php echo JHtml::_('jgrid.checkedout', $i, $item->editor, $item->locked_on, 'rules.', $canCheckin); ?>
									<?php endif; ?>

									<?php if (!$item->locked_by && $canChange || $item->locked_by === $loggeduser->id)
									{
										echo JHtml::_(
											'link',

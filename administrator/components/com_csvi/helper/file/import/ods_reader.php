...
class ODSParser
{
	...
	// Get the last array key element
	private $lastKeyElement = 0;
	...
	* @todo parse <text:s text:c="2"/>
	...
	private function characterData($parser, $data) {
		...
		if (isset($this->_data[$this->_linecount]['options']))
		{
			foreach ($this->_data[$this->_linecount]['options'] as $option => $value)
			{
				switch ($option)
		...
		//XML parser breaks content with accent characters so collect and put them togather
		$newArray = array_filter($this->_data[$this->_linecount], 'is_int', ARRAY_FILTER_USE_KEY);
		end($newArray);
		$this->lastKeyElement = key($newArray);

		if (isset($this->_data[$this->_linecount]['data']) && $this->_data[$this->_linecount]['data'])
		{
			$newData                                               = $this->_data[$this->_linecount][$this->lastKeyElement] . $data;
			$this->_data[$this->_linecount][$this->lastKeyElement] = $newData;
			unset($this->_data[$this->_linecount][$this->lastKeyElement + 1]);
		}
		else
		{
			$this->_data[$this->_linecount][] = $data;

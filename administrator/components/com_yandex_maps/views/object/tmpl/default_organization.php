		...
		<div class="row-fluid">
			...
			<div class="span6">
				<div id="jform_organization_acting_basis_params" style="display:none">
					...
					<div class="control-group">
						<label>Дата</label>
						<input class="validate fdate <?php echo $params->get('registration_organization_acting_basis_date', 2)==2 ? 'required' : ''?>" data-rules="date" name="jform[organization_acting_basis_date]"   value="<?php echo htmlspecialchars(@$organization->organization_acting_basis_date)?>" id="jform_organization_acting_basis_date"  type="text" placeholder="Например: 12.12.2009"/>
	...
	<div>
		<div class="row-fluid">
			<div class="span6 ">
				...
				<div class="control-group">
					<label>Телефон</label>
					<input data-format="<?php echo $params->get('registration_phone_format', '+7 (9##) ###-##-##')?>" class="phone validate <?php echo $params->get('registration_organization_contact_phone', 2)==2 ? 'required' : ''?>" data-rules="phone" name="jform[organization_contact_phone]" value="<?php echo htmlspecialchars(@$organization->organization_contact_phone)?>" id="jform_organization_contact_phone" type="text" placeholder="Например: +79177378599"/>
					<div class="xdsoft_tooltip">Введите номер телефона в следующем формате <?php echo $params->get('registration_phone_format', '+7 (9##) ###-##-##')?></div>
				</div>
				<?php } ?>
			</div>
				...
				foreach ($var_adreses as $type=>$label) {
					...
					<div class="span6" id="form_organization_address<?php echo $type?>">
						...
						<?php switch ($params->get('registration_organization_address_input_view',1)) {
							case 1:
								include JPATH_ROOT.'/components/com_yandex_maps/views/registration/tmpl/address_in_oneline.php';
							break;
							case 0:
								include JPATH_ROOT.'/components/com_yandex_maps/views/registration/tmpl/address_in_parts.php';
							break;
						}?>
					</div>
				<?php }
...
<fieldset>
	...
	<div class="control-group">
		<label>Номер</label>
		<input data-format="<?php echo $params->get('registration_organization_phone_format', '+7 (9##) ###-##-##')?>" class="phone validate <?php echo $params->get('registration_organization_phone', 2)==2 ? 'required' : ''?>" data-rules="phone" name="jform[organization_phone]"  value="<?php echo htmlspecialchars(@$organization->organization_phone)?>" id="jformorganization__phone" type="text" placeholder="Например: +79177378599"/>
...
<fieldset>
	...
	<div>
		<div class="row-fluid">
			...
			<div class="span6 control-group">
				<label>Дата</label>
				<input class="fdate validate <?php echo $params->get('registration_organization_license_date', 2)==2 ? 'required' : '';?>" data-rules="date" name="jform[organization_license_date]"  value="<?php echo htmlspecialchars(@$organization->organization_license_date)?>"  id="jform_organization_license_date" type="text" placeholder="Например: 12.05.2007"/>
				<div class="xdsoft_tooltip">Введено неверно. Возможно не верный формат даты</div>
			</div>
			<?php } ?>
		</div>
	...
	<div>
		<div class="row-fluid">
			...
			<div class="span6 control-group">
				...
				<div class="xdsoft_tooltip">Введено неверно. Не верный адрес URL. Введите полный url сайта, к примеру: https://xdan.ru</div>
			</div>
			<?php } ?>
		</div>

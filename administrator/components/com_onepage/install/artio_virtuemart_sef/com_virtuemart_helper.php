...
class vmrouterHelperSEF {
	...
	private function __construct($instanceKey,$query) {
		...
		
		{
			...
			$this->seo_sufix_size = 0; //strlen($this->seo_sufix) ;
	...
	public static function getInstance(&$query = null) {
		...
		if (! array_key_exists ($instanceKey, self::$_instances)){
			self::$_instances[$instanceKey] = new vmrouterHelperSEF ($instanceKey,$query);
	...
	public function getCategoryNames($virtuemart_category_id,$catMenuId=0, $parents=0){
		...
		if (empty($this->vmlang))
		{
			die('empty vmlang'.$this->vmlang); 
		}
	...
	public function getCategoryRecurse($virtuemart_category_id,$catMenuId,$first=true ) {
		...
		if (isset ($ids->child)) {
			...
			if($ids->parent != 0 and $catMenuId != $virtuemart_category_id and $catMenuId != $ids->parent) {
				$this->getCategoryRecurse($ids->parent,$catMenuId,$first);
			}
		}
		return $idsArr ;
	}
	...
	{
	  $option = (int)$params->get('details_path', 3);
	  if ($option == 5)
	  {
	...
	public function getProductId($names,$virtuemart_category_id = NULL ){
		...
		$productName =  $productName; //substr($productName, 0, -(int)$this->seo_sufix_size );
		$product = array();
	...
	public function getManufacturerId($slug ){
		$db = JFactory::getDBO();
		$query = "SELECT `virtuemart_manufacturer_id` FROM `#__virtuemart_manufacturers_".$this->vmlang."` WHERE `slug` LIKE '".$db->getEscaped($slug)."' limit 0,1";
		//$db->setQuery($query);
  ...
  // pozor, zle zoradene! pouziva sa iba pri klucovych slovach
   ...
   return $res;
	...
	function getProductKeywords($product_id, $title, $lang)
	{
	 ...
	 //var_dump($this->product_paths[$product_id]); die(); 
	 $kw = ''; 
	 foreach ($title as $w)
	  {
	    if (!empty($kw)) $kw .=','; 

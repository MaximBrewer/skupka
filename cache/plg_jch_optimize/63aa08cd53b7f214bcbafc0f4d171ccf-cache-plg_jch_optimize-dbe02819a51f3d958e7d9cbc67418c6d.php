<?php die("Access Denied"); ?>#x#a:2:{s:6:"result";s:31898:"try{var RSFormProCalendars={};var RSFormProPrices={};var ajaxExtraValidationScript={};var RSClickedSubmitElement=false;if(typeof RSFormPro!='object'){var RSFormPro={};}
...
{RSFormProUtils.addEvent(resetElements[i],'click',function(){window.setTimeout(function(){RSFormPro.Conditions.runAll(formId);},1);});}}}};RSFormPro.Calculations={addEvents:function(formId,fields){var func=window["rsfp_Calculations"+formId];var thefields=fields?fields:RSFormProPrices;var isIE8=navigator.userAgent.match(/MSIE 8\.0/);var event='click';var resetElements=RSFormPro.getElementByType(formId,'reset');if(resetElements.length>0)
{for(var i=0;i<resetElements.length;i++)
...
if(typeof data.parentErrorClass!='undefined'&&data.parentErrorClass.length>0){try{elementBlock=RSFormPro.getBlock(formId,RSFormProUtils.getAlias(formComponents[id]));RSFormProUtils.addClass(elementBlock[0],data.parentErrorClass);}catch(err){}}
if(typeof data.fieldErrorClass!='undefined'&&data.fieldErrorClass.length>0){try{results=RSFormPro.getFieldsByName(formId,formComponents[id]);if(results.length>0)
{for(var r=0;r<results.length;r++)
{RSFormProUtils.addClass(results[r],data.fieldErrorClass);}}}catch(err){}}}}}}}
...
function ajaxValidation(form,page,errorClasses){return RSFormPro.Ajax.validate(form,page,errorClasses);}}catch(e){console.error('Error in file:/media/com_rsform/js/script.js; Error:'+e.message);};";s:6:"output";s:0:"";}
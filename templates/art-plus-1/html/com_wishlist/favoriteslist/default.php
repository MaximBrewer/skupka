...
$idCustomfield = array(115, 117, 118, 119, 120, 121, 122, 123, 124, 125, 126);
...
JHtml::stylesheet('https://netdna.bootstrapcdn.com/font-awesome/4.0.3/css/font-awesome.css', array(), false);
...
$my_page->setTitle($sitename. ' ' .JText::_( 'Избранные товары' ));
...
//		var_dump($this->products);
...

    {
        ...
        foreach ($this->products as $it){
            // echo "<pre>";
            // var_dump($dataItem->product_id);
            // var_dump($it->virtuemart_product_id);
            // echo "</pre>";
            if($dataItem->product_id==$it->virtuemart_product_id){
                $product=$it;
            }
        }
        ...
        $form_deletefavorite .= '<button class="modns button art-button art-button" title="'.JText::_('').'" >';

        if ($tmpl_favbtn_image) $form_deletefavorite .= '<i class="fa fa-trash-o"></i>';

        $form_deletefavorite .= JText::_('Удалить из избранного').'</button>';
...
//				    echo "<h4 class='prod_price'>".$currency->createPriceDiv('salesPrice','',$product->prices,true)."</h4>";
...
//				FavoritesModelFavoriteslist::addtocart($product, JText::_('COM_VIRTUEMART_CART_ADD_TO'),$product_ord);
        ...
        ?>
         <div class="tovar_zakaz_v_klik">
            ...
            <a data-link="<?php echo $link_;?>" data-name="<?=$this->product->product_name ?>"  class="tovar_zakaz" href="index.php?option=com_rsform&formId=22&tmpl=component">Оптовая<br>цена</a>
        </div>
        ...
        echo "</div>";
 echo "<ul class='customfield_block'>";
        foreach ($product->customfieldsSorted as $key=>$items){
                 if($items){
                    foreach ($items as $item) {
                         if(in_array($item->virtuemart_custom_id,$idCustomfield)){
                            echo '<li><strong>'.$item->custom_title.': </strong><span>'.$item->customfield_value.'</span></li>';
                         }
                    }
                 }
        }

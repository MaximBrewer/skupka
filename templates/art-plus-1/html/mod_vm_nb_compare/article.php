...
//сюда пишем значения для дополнительных полей
$custom_field_product=array();
$product_key=array();
...
foreach ($_SESSION['compare'] as $key => $product_id) {
    ...
    if ($show_prices == '1') {
        ...
        $currencyHtml3 = $currency->createPriceDiv('priceWithoutTax', ' ', $product->prices);
    ...
    foreach ($product->customfields as $value) {
        if($value->is_cart_attribute){
            ...
            $custom_field_product[$key][$value->custom_title] = $value->display;
        }
...
//    $images[$key] = $http.$product->images[0]->file_url_thumb; // *!* 2016-08-21
    ...
    if($product->prices['salesPrice'] != $product->prices['priceWithoutTax']){
        $ar_price=array($currencyHtml3,$currencyHtml4);
    }else{
        $ar_price=array($currencyHtml4);
    }
    $price[$key] = $ar_price;
...
//проверяем поля
$ar_key_custom=array();
foreach ($custom_field_product as $items){
       foreach ($items as $key => $value) {
          array_push($ar_key_custom,$key) ;
       }
}
$ar_key_custom_unig = array_unique($ar_key_custom);
...
<!-- скрипты virtualmart -->
<script src="<?php echo $http ?>components/com_virtuemart/assets/js/vmprices.js"></script>
<script id="jsVars_js" type="text/javascript">
     usefancy = true;
</script>
    ...
    <table class="popup_table">
        ...
        <tr class="tr_compare_price">
            ...
            <?php foreach ($price as $key => $value) { ?>
                <td class="price_compare product_compare_item <?php echo $key ?>">
                    <?php foreach ($value as $item){
                        if(!empty($item)) echo $item;
                    } ?>
                    <?php //echo $value ?>
                </td>
            <?php } ?>
        ...
        <tr class="tr_compare_availability">
            ...
            <?php foreach ($availability as $key => $value) { ?>
                <td class="availability_compare product_compare_item <?php echo $key ?>">
                    ...
                    if ($value) {
                        ?>
                        <span class="availability_TF">В наличии</span><span
                                class="availability_item"><?php echo $value ?> item(s)</span>
                        <?php
                    } else {
                        ?>
                ...
                <form method="post" class="product" action="<?php echo JRoute::_('/compare/', false); ?>">
                    <div class="addtocart-bar-plg">
                            ...
                            if (count($customfields)>0) {
                                foreach ($customfields as $key => $customfield) {
                                    ...
                                    if($customfield->is_cart_attribute){
                                        ...
                                        echo '<span class="custom_title">'.$customfield->custom_title.'</span>';
                        ...
                        <div class="quantity-box_1">
                          ...
                          <div class="bistr"> <!-- кнопка быстрый заказ -->
<?php
 $link_=JURI::base().JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$product->virtuemart_product_id.'&virtuemart_category_id='.$product->virtuemart_category_id);
 ?>
 <style type="text/css">
      /*скрываем ссылку которую вывели для подгрузки*/
     [href="#hidden_link"]{
 ...
 <div>
     <a style="display: inline-block;" data-link="<?php echo $link_;?>" data-name="<?=$product->product_name ?>"  class="tovar_zakaz" href="index.php?option=com_rsform&formId=22&tmpl=component">Купить в 1 клик</a>
 </div>
<!-- кнопка быстрый заказ --></div>
                      </div>
...
<style type="text/css">
    ...
    .tr_compare_manufacturer {
        display: none
    }

    <?php } ?>
    <?php if(!$availabilitys){ ?>
    .tr_compare_availability {
        display: none
    }
